...
public class LifeExpectancyProcessor implements EntityDocumentProcessor {
	...
	public void processItemDocument(ItemDocument itemDocument) {
		...
		if (birthYear != Integer.MIN_VALUE && deathYear != Integer.MIN_VALUE && birthYear >= 1200) {
	...
	public void writeFinalResults() {
		...
		try (PrintStream out = new PrintStream(ExampleHelpers.openExampleFileOuputStream("life-expectancies.csv"))) {

			for (int i = 0; i < lifeSpans.length; i++) {
				if (peopleCount[i] != 0) {
					out.println(i + "," + (double) lifeSpans[i] / peopleCount[i] + "," + peopleCount[i]);
				}
			}
		} catch (IOException e) {
			e.printStackTrace();
	...
	private void printStatus() {
		if (this.totalPeopleCount != 0) {
			System.out.println("Found " + totalPeopleCount + " people with an average life span of "
	...
	private int getYearValueIfAny(StatementGroup statementGroup) {
		...
		for (Statement s : statementGroup.getStatements()) {
			...
			if (s.getClaim().getMainSnak() instanceof ValueSnak) {
				...
				if (v instanceof TimeValue && ((TimeValue) v).getPrecision() >= TimeValue.PREC_YEAR) {
