...
class EntityStatisticsProcessor implements EntityDocumentProcessor {
	...
	protected void countTerms(UsageStatistics usageStatistics, TermedDocument termedDocument) {
		...
		for (MonolingualTextValue mtv : termedDocument.getDescriptions().values()) {
			countKey(usageStatistics.descriptionCounts, mtv.getLanguageCode(), 1);
		}
		...
		for (StatementGroup sg : statementDocument.getStatementGroups()) {
			...
			for (Statement s : sg.getStatements()) {
				for (SnakGroup q : s.getClaim().getQualifiers()) {
					countPropertyQualifier(usageStatistics, q.getProperty(), q.getSnaks().size());
				}
				for (Reference r : s.getReferences()) {
					usageStatistics.countReferencedStatements++;
					for (SnakGroup snakGroup : r.getSnakGroups()) {
						countPropertyReference(usageStatistics, snakGroup.getProperty(), snakGroup.getSnaks().size());
					}
				}
			}
		}
	...
	private void writeFinalResults() {
		...
		writePropertyStatisticsToFile(this.itemStatistics, "item-property-counts.csv");
		writePropertyStatisticsToFile(this.propertyStatistics, "property-property-counts.csv");

		// Store site link statistics in file:
		try (PrintStream out = new PrintStream(ExampleHelpers.openExampleFileOuputStream("site-link-counts.csv"))) {

			out.println("Site key,Site links");
			for (Entry<String, Integer> entry : this.siteLinkStatistics.entrySet()) {
		...
		writeTermStatisticsToFile(this.propertyStatistics, "property-term-counts.csv");
	}
	 ...
	 */
	private void writePropertyStatisticsToFile(UsageStatistics usageStatistics, String fileName) {
		try (PrintStream out = new PrintStream(ExampleHelpers.openExampleFileOuputStream(fileName))) {
			...
			for (Entry<PropertyIdValue, Integer> entry : usageStatistics.propertyCountsMain.entrySet()) {
				int qCount = usageStatistics.propertyCountsQualifier.get(entry.getKey());
				int rCount = usageStatistics.propertyCountsReferences.get(entry.getKey());
	 ...
	 *            the name of the file to use
	 */
	private void writeTermStatisticsToFile(UsageStatistics usageStatistics, String fileName) {
		...
		try (PrintStream out = new PrintStream(ExampleHelpers.openExampleFileOuputStream(fileName))) {
			...
			for (Entry<String, Integer> entry : usageStatistics.labelCounts.entrySet()) {
				...
				int dCount = usageStatistics.descriptionCounts.get(entry.getKey());
				out.println(entry.getKey() + "," + entry.getValue() + "," + dCount + "," + aCount);
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
	 ...
	 *            "properties")
	 */
	private void printStatistics(UsageStatistics usageStatistics, String entityLabel) {
		System.out.println("Processed " + usageStatistics.count + " " + entityLabel + ":");
		System.out.println(" * Labels: " + usageStatistics.countLabels + ", descriptions: "
				+ usageStatistics.countDescriptions + ", aliases: " + usageStatistics.countAliases);
		System.out.println(" * Statements: " + usageStatistics.countStatements + ", with references: "
				+ usageStatistics.countReferencedStatements);
	}
	 ...
	 */
	private void countPropertyMain(UsageStatistics usageStatistics, PropertyIdValue property, int count) {
		addPropertyCounters(usageStatistics, property);
		usageStatistics.propertyCountsMain.put(property, usageStatistics.propertyCountsMain.get(property) + count);
	}
	 ...
	 *            the number of times to count the property
	 */
	private void countPropertyQualifier(UsageStatistics usageStatistics, PropertyIdValue property, int count) {
		addPropertyCounters(usageStatistics, property);
	 ...
	 *            the number of times to count the property
	 */
	private void countPropertyReference(UsageStatistics usageStatistics, PropertyIdValue property, int count) {
		addPropertyCounters(usageStatistics, property);
	 ...
	 *            the property to count
	 */
	private void addPropertyCounters(UsageStatistics usageStatistics, PropertyIdValue property) {
